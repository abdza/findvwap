...
def backtest():
    ...
    # for i in range(10):
        ...
        if len(candles.index):
            ...
            for i in range(len(candles)):
                ...
                if minute_test(peaks,bottoms):
                    passminute += 1
                if profit_test(peaks,bottoms):
                    gotprofit += 1
                if minute_profit_test(peaks,bottoms):
                    minuteprofit += 1
                    minuteprofittickers.append(ticker)
                if minute_test(peaks,bottoms) and prev_avg_test(minute_candles,bbminute_candles) and prev_avg_test(minute_candles,bminute_candles,5):
    ...
    print("High Openers:")
    return highrange,fullrange,stats
...
if 'cal' not in st.session_state:
    ...
    caldata = []
    highrange,fullrange,stats = backtest()
