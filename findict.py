    ...
    if len(candles.index):
        ...
        if cdcandle.iloc[0]['open'] > prevcandle.iloc[-1]['close']:
            ...
            if valuegap>0:
                ...
                curdata = {'Ticker':ticker,'Gap':gap,'Value Gap':valuegap,'End':prevcandle.iloc[-1]['date'],'Start':cdcandle.iloc[0]['date']}
                if lastcandle['date'].hour>=15 and lastcandle['date'].minute>=30:   #it is past 3.30 so consider the day closed
                    curdata['Day Range'] = lastcandle['close'] - cdcandle.iloc[0]['open']
                    curdata['Open Diff'] = lastcandle['high'] - cdcandle.iloc[0]['high']
                highrangetickers.append(curdata)

        if cdcandle.iloc[0]['open'] < prevcandle.iloc[-1]['close']:
            gap = prevcandle.iloc[-1]['close'] - cdcandle.iloc[0]['open']
            valuegap = prevcandle.iloc[-1]['low'] - cdcandle.iloc[0]['high']
            if valuegap>0:
                print("Gap:",gap)
                lastcandle = cdcandle.iloc[-1]
                curdata = {'Ticker':ticker,'Gap':gap,'Value Gap':valuegap,'End':prevcandle.iloc[-1]['date'],'Start':cdcandle.iloc[0]['date']}
                if lastcandle['date'].hour>=15 and lastcandle['date'].minute>=30:   #it is past 3.30 so consider the day closed
                    curdata['Day Range'] = lastcandle['close'] - cdcandle.iloc[0]['open']
                    curdata['Open Diff'] = lastcandle['high'] - cdcandle.iloc[0]['high']
                lowrangetickers.append(curdata)
        ...
        if minbottom is not None:
            if minbottom['high']<cdcandle.iloc[-1]['close']:
                maxrange = cdcandle.iloc[-1]['close'] - minbottom['high']
                curdata = {'Ticker':ticker,'Range':maxrange,'Bottom':minbottom['date'],'Low':minbottom['high'],'High':cdcandle.iloc[-1]['close']}
                wintickers.append(curdata)
...
print("Low range tickers")
print(tabulate(lowrangetickers,headers="keys"))
print("Winning tickers")
print(tabulate(wintickers,headers="keys"))
for tic in wintickers:
    crossdata = None
    if tic['Ticker'] in [tt['Ticker'] for tt in lowrangetickers]:
        crossdata = {'Ticker':tic['Ticker'],'Status':'Low','Range':tic['Range']}
    if tic['Ticker'] in [tt['Ticker'] for tt in highrangetickers]:
        crossdata = {'Ticker':tic['Ticker'],'Status':'High','Range':tic['Range']}
    if crossdata is not None:
        crosstickers.append(crossdata)
print("Cross tickers")
print(tabulate(crosstickers,headers="keys"))
